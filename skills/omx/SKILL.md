...
```bash
op_pi deliver --session <tmux-session> --prompt "..." --max-enters 4
```

Legacy `op_pi hooks install --scope project` flows are migration-only shims; rerun the default global install path for supported setups.
