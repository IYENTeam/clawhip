...
```bash
op-pi deliver --session <tmux-session> --prompt "..." --max-enters 4
```

Legacy `op-pi hooks install --scope project` flows are migration-only shims; rerun the default global install path for supported setups.
