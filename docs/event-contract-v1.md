...
This document freezes op_pi's v1 shared provider-native hook contract for Codex and Claude.
...
op_pi v1 supports exactly these five provider-native events:
...
Within the frozen shared event family, op_pi treats `PreToolUse`/`PostToolUse` calls to
...
After op_pi normalizes a provider payload, these base routing fields are the v1 contract:
...
`.op_pi/hooks/` may only add data to the normalized base contract.
