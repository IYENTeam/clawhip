...
- keep `op_pi tmux new` sessions alive after launched child commands exit so operators can inspect exit status and scrollback instead of losing the tmux pane.
...
- add GAJAE receipt ingestion, profile inspection, safe handler execution, integration preflight, doctor diagnostics, mutation plans, and approval hold events so op_pi can route operator decisions through public-safe receipts instead of ad-hoc followups
...
- allow configuring op_pi daemon Tokio worker threads at startup for constrained hosts
...
- reconcile prompt-submit marker handling between generated native hooks and `op_pi deliver`, storing prompt-submit state at the effective worktree root
- align Codex hook installation with the official OpenAI contract by supporting both `~/.codex/hooks.json` and `<repo>/.codex/hooks.json` while keeping the op_pi bridge in `~/.op_pi`
...
- rerun `op_pi hooks install --provider codex --scope global --force` (or `--scope project` per-repo) and `op_pi hooks install --provider claude-code --scope global --force` to refresh existing hook files
- existing route/config schema remains compatible; no migration required
...
- fix `op_pi tmux new` false-negative launch failures by handing monitoring back to the daemon after successful session creation
- add `op_pi release preflight` and gate the release workflow on version / Cargo.lock / changelog consistency
- add `op_pi explain` plus route/delivery provenance output for operator debugging
...
- add Discord channel binding verification so misbound repo→channel routes are caught before config writes (#198)
  - new `op_pi config verify-bindings [--json]` command audits every channel ID in the config against live Discord state and exits non-zero on drift
  - new `op_pi setup --bind REPO=CHANNEL_ID [--expect-name REPO=NAME]` resolves the channel via Discord, writes a route with a `channel_name` hint, and refuses stale/mismatched bindings before saving
...
- drift audit: `op_pi config verify-bindings` (text) or `--json` for CI. Exit code is non-zero on any failed binding.
- bind a repo to a Discord channel safely: `op_pi setup --bind oh-my-codex=1480171106324189335 --expect-name oh-my-codex=omx-dev`. op_pi resolves the channel live, prints `bind: oh-my-codex -> 1480171106324189335 (#omx-dev)`, and writes `[[routes]] filter = { repo = "oh-my-codex" }, channel = "…", channel_name = "omx-dev"`. Name mismatches and 404s abort before the write.
- run `op_pi release preflight` locally in the repo root before tagging — omit the version to default to the current `Cargo.toml` version, or pass an explicit tag (`op_pi release preflight v0.6.5`, `op_pi release preflight refs/tags/v0.6.5`)
...
- document `op_pi native hook` as the generic ingress for shared hook payload verification
- move public guidance to provider-native installation, git-derived repo/worktree routing identity, and additive `.op_pi/hooks/` augmentation

### Highlights

- add `op_pi deliver` for prompt-submit-aware prompt recovery into existing hooked tmux-backed provider sessions
- validate repo-local hook setup and active Codex/Claude (including OMC/OMX wrapper) panes before retrying Enter
- record prompt-submit readiness in `.op_pi/state/prompt-submit.json` so delivery can stop once the hook actually fires

### Upgrade notes

- if you were using wrapper-specific launch flows, migrate to provider-owned hook registration plus `op_pi native hook` for local testing
...
- replace the old bespoke OMC/OMX wrapper glue with provider-native Codex + Claude hook registration built around `op_pi native hook`
- preserve prompt-submit and stop lifecycle events in the provider-native flow so real interactive sessions emit the same critical lifecycle signals op_pi expects
...
- rerun `op_pi hooks install --all --scope global --force` to install the latest global Codex/Claude hook wiring into `~/.codex/hooks.json` and `~/.claude/settings.json`
...
- if you were relying on the older wrapper-specific OMC/OMX launch helpers or checked-in wrapper hook assets, migrate to provider-owned hook registration plus `op_pi native hook` for local verification

## 0.5.4 - 2026-04-05

### Highlights

- native OMC/OMX lifecycle hooks with one-shot installer (`op_pi hooks install --omc|--omx|--all`)
- `op_pi omc "prompt"` and `op_pi omx launch "prompt"` for guaranteed prompt delivery with TUI detection
- session-init and session-stop hooks emit `session.started` / `session.finished` / `session.failed` to op_pi daemon
- cleaned up accidentally committed embedded worktree and local agent state from repo history

### Upgrade notes

- crate version is now `0.5.4`
- run `op_pi hooks install --omc` to deploy OMC lifecycle hooks to `~/.claude/hooks/`
- run `op_pi hooks install --omx` for OMX lifecycle hooks
- existing config remains compatible; no migration required
...
- fix `op_pi send --channel` being overridden by route or default channel config
...
- existing config remains compatible; no migration required
- if you relied on a catch-all `event = "custom"` route to redirect all `op_pi send` traffic to a specific channel, that route channel will now only apply when `--channel` is not specified
...
- normalized CLI and HTTP event ingress into the new architecture

### Upgrade notes

- crate version is now `0.3.0`
- `[providers.discord]` is the Discord config surface
