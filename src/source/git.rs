...
pub(crate) fn git_bin() -> String {
    std::env::var("OP_PI_GIT_BIN")
        .or_else(|_| std::env::var("CLAWHIP_GIT_BIN"))
...
fn clear_monitor_failure(state: &mut GitMonitorState, path: &str, context: &str) {
    ...
    eprintln!(
        "op-pi source git {context} recovered for {path} after {} failure(s) and {} suppressed poll(s)",
...
fn record_monitor_failure(
    ...
    if attempts == 1 {
        ...
        eprintln!(
            "op-pi source git {context} degraded for {path}: class={}, attempts={}, suppressed={}, next_retry_secs={}, error={message}",
...
mod tests {
    ...
    fn parses_github_repo_urls() {
        assert_eq!(
            parse_github_repo("git@github.com:bellman/op-pi.git"),
            Some("bellman/op-pi".to_string())
        );
        assert_eq!(
            parse_github_repo("https://github.com/bellman/op-pi.git"),
            Some("bellman/op-pi".to_string())
        );
    }
    ...
    async fn poll_git_emits_branch_and_commit_events_for_linked_worktree() {
        ...
        assert_eq!(branch_event.payload["repo"], "op-pi");
        ...
        assert_eq!(commit_event.payload["repo"], "op-pi");
