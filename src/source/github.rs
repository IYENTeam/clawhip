...
impl Source for GitHubSource {
    ...
    async fn run(&self, tx: mpsc::Sender<IncomingEvent>) -> Result<()> {
        let github_client = match build_github_client(self.config.monitor_github_token()) {
            ...
            Err(error) => {
                eprintln!("op_pi source github: failed to build GitHub client: {error}");
                None
            }
        };
        ...
        loop {
            ...
            .await
            {
                ...
                Err(error) => {
                    ...
                    if error_logs.should_log("poll", &error) {
                        ...
                        eprintln!("op_pi source github poll failed: {error}");
                    }
                }
            }
            ...
            if let Err(error) = save_state(&state_path, &state).await {
                eprintln!("op_pi source github failed to save state: {error}");
            }
    ...
    match tokio::fs::read_to_string(path).await {
        Ok(content) => {
            ...
            serde_json::from_str(&content).unwrap_or_else(|error| {
                eprintln!(
                    "op_pi source github: failed to parse state file at {}: {error}",
                    path.display()
                );
                HashMap::new()
            })
        ...
        Err(error) => {
            eprintln!(
                "op_pi source github: failed to read state file at {}: {error}",
                path.display()
            );
            HashMap::new()
        }
    ...
    match snapshot_git_repo(repo).await {
        ...
        Err(error) => match repo.github_repo.clone() {
            Some(github_repo) => {
                ...
                eprintln!(
                    "op_pi source github snapshot failed for {}: {error}; using configured github_repo={github_repo}",
                    repo.path
                );
                Ok(GitSnapshot {
    ...
    for repo in &config.monitors.git.repos {
        ...
        let snapshot = match snapshot_github_repo(repo).await {
            ...
            Err(error) => {
                ...
                eprintln!(
                    "op_pi source github snapshot failed for {}: {error}",
                    repo.path
                );
...
async fn run_reconciliation(
    ...
    for repo in &config.monitors.git.repos {
        ...
        let snapshot = match snapshot_github_repo(repo).await {
            ...
            Err(error) => {
                eprintln!(
                    "op_pi source github reconciliation: snapshot failed for {}: {error}",
                    repo.path
                );
                continue;
            }
...
fn build_github_client(token: Option<String>) -> Result<reqwest::Client> {
    ...
    headers.insert(USER_AGENT, HeaderValue::from_static("op_pi/0.1"));
    headers.insert(
        ACCEPT,
...
mod tests {
    ...
    async fn new_issue_events_apply_route_channel_and_mention_over_repo_monitor_channel() {
        ...
        .into_iter()
        .collect();
        let events = collect_issue_events(&repo, "op_pi", &previous, &current);
        assert_eq!(events.len(), 1);
        assert_eq!(events[0].canonical_kind(), "github.issue-opened");
        assert_eq!(events[0].payload["repo"], "op_pi");
    ...
    fn pr_review_activity_events_are_emitted() {
        ...
        let events = collect_pr_events(&repo, "op_pi", &previous, &current);
    ...
    fn ci_snapshot(
        ...
        GitHubCISnapshot {
            ...
            url: "https://github.com/IYENTeam/op_pi/actions/runs/1".into(),
    ...
    fn initial_ci_detection_emits_started_event_with_route_metadata() {
        ...
        let events = collect_ci_events(&repo, "op_pi", false, &previous, &current);
        assert_eq!(events.len(), 1);
        ...
        assert_eq!(events[0].payload["repo"], json!("op_pi"));
        ...
        assert_eq!(
            ...
            json!("https://github.com/IYENTeam/op_pi/actions/runs/1")
        );
    }
    ...
    fn absent_terminal_ci_after_baseline_emits_completion_events() {
        ...
        for (conclusion, expected_kind) in [
            ...
            let events = collect_ci_events(&repo, "op_pi", true, &previous, &current);
            assert_eq!(
                events.len(),
                1,
    ...
    async fn snapshot_falls_back_to_configured_github_repo_without_local_clone() {
        let repo = GitRepoMonitor {
            path: "/tmp/op_pi-test-private-repo-missing".into(),
