...
//! 2. `Cargo.lock` already records that version for the `op-pi` package
...
///
/// Accepts `1.2.3`, `v1.2.3`, `op-pi-v1.2.3`, and `refs/tags/v1.2.3` —
...
"#;
    ...
    fn normalize_version_accepts_common_tag_shapes() {
        ...
        assert_eq!(normalize_version("op-pi-v0.6.5"), "0.6.5");
        assert_eq!(normalize_version("op-pi/v0.6.5"), "0.6.5");
    }
    ...
    fn check_cargo_lock_passes_when_package_version_matches() {
        let result = check_cargo_lock(CARGO_LOCK_SAMPLE, "op-pi", "0.6.5");
        assert!(result.passed, "detail = {}", result.detail);
    }
    ...
    fn check_cargo_lock_fails_when_lock_is_stale() {
        ...
        let result = check_cargo_lock(&stale_lock, "op-pi", "0.6.5");
        assert!(!result.passed);
    ...
    fn check_cargo_lock_fails_when_package_missing() {
        let result = check_cargo_lock("# empty", "op-pi", "0.6.5");
        assert!(!result.passed);
...
version = "0.6.4"
"#;
        let result = check_cargo_lock(lock, "op-pi", "0.6.5");
