...
pub fn verify_secret(provided: Option<&str>, expected: Option<&str>) -> Result<(), IntakeError> {
    ...
    let provided = provided
        .ok_or_else(|| IntakeError::Unauthorized("missing shared secret header".into()))?;
...
impl SnsCertCache {
    ...
    fn get(&self, url: &str) -> Option<RsaPublicKey> {
        ...
        let now = std::time::SystemTime::now()
            ...
            .ok()?
            .as_secs() as i64;
        (entry.fetched_at.elapsed().as_secs() < SNS_CERT_CACHE_TTL_SECS && now < entry.not_after_unix)
...
pub fn sns_canonical_string(payload: &Value) -> Result<String, IntakeError> {
    ...
    let fields: &[&str] = match message_type {
        ...
        other => return Err(IntakeError::BadRequest(format!("unsupported SNS Type '{other}'"))),
    };
...
pub fn validate_signing_cert_url(url: &str) -> Result<(), IntakeError> {
    let parsed = reqwest::Url::parse(url)
        .map_err(|error| IntakeError::Unauthorized(format!("SigningCertURL is not a URL: {error}")))?;
...
pub fn parse_signing_cert(pem: &str) -> Result<(RsaPublicKey, i64), IntakeError> {
    ...
    let (_, pem_block) = x509_parser::pem::parse_x509_pem(pem.as_bytes())
        .map_err(|error| IntakeError::Unauthorized(format!("invalid signing certificate: {error}")))?;
    let cert = pem_block
        .parse_x509()
        .map_err(|error| IntakeError::Unauthorized(format!("invalid signing certificate: {error}")))?;
    check_cert_validity(
    ...
    let key = RsaPublicKey::from_public_key_der(cert.public_key().raw)
        .map_err(|error| IntakeError::Unauthorized(format!("certificate key is not RSA: {error}")))?;
    Ok((key, not_after))
}

/// Verify the SNS message signature against a known public key.
pub fn verify_sns_signature_with_key(payload: &Value, key: &RsaPublicKey) -> Result<(), IntakeError> {
    ...
    let signature = base64::engine::general_purpose::STANDARD
        ...
        .map_err(|error| IntakeError::Unauthorized(format!("invalid SNS Signature encoding: {error}")))?;
    let version = payload
...
pub async fn verify_sns_signature(payload: &Value, cache: &SnsCertCache) -> Result<(), IntakeError> {
    let url = payload
        .get("SigningCertURL")
        .and_then(Value::as_str)
...
mod tests {
    ...
    fn topic_allowlist_enforced() {
        ...
        assert!(topic_allowed(&[], "arn:aws:sns:us-east-1:999999999999:anything"));
    }
    ...
    fn cf_notification_maps_alert_type_and_preserves_data() {
        ...
        assert_eq!(event.payload["text"], "Health check origin-api is unhealthy");
    }
    ...
    fn cf_logpush_ndjson_batch_splits_records_and_caps() {
        ...
        for i in 0..120 {
            lines.push(format!("{{\"RayID\":\"ray-{i}\",\"EdgeResponseStatus\":403}}"));
        }
        let body = lines.join("\n");
    ...
    fn cf_logpush_gzip_body_decodes() {
        ...
        let event =
            normalize_cf_logpush_batch("audit_logs_v2", &gzipped, Some("gzip")).unwrap();
...
mod sns_signature_tests {
    ...
    fn signing_cert_url_validation() {
        ...
        assert!(validate_signing_cert_url("https://sns.evil.amazonaws.com.evil.com/x.pem").is_err());
    ...
    fn signing_cert_url_rejects_authority_smuggling() {
        ...
        .is_err());
        assert!(validate_signing_cert_url("https://sns.us-east-1.amazonaws.com:444/x.pem").is_err());
        assert!(validate_signing_cert_url("https://user@sns.us-east-1.amazonaws.com/x.pem").is_err());
    }
