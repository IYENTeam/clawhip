...
pub async fn run(
    ...
    println!("op-pi v{VERSION} starting (token_source: {token_source})");
    ...
    tokio::spawn(async move {
        ...
        if let Err(error) = dispatcher.run().await {
            eprintln!("op-pi dispatcher stopped: {error}");
        }
    });
    spawn_source(
    ...
    println!(
        "op-pi daemon v{VERSION} listening on http://{} (token_source: {token_source})",
        local_addr
    );
    telemetry::emit(daemon_record(
    ...
    S: Source + Send + Sync + 'static,
{
    ...
    tokio::spawn(async move {
        println!("op-pi source '{}' starting", source_name);
        ...
        match source.run(tx.clone()).await {
            ...
            Err(error) => {
                ...
                eprintln!("op-pi source '{}' stopped: {error}", source_name);
                if let Err(alert_error) = tx
                    .send(source_failure_alert_event(&source_name, &error.to_string()))
                    .await
                {
                    eprintln!(
                        "op-pi source '{}' could not enqueue degraded alert: {alert_error}",
                        source_name
                    );
                }
            }
...
fn source_failure_alert_event(source_name: &str, error_message: &str) -> IncomingEvent {
    let mut event = IncomingEvent::custom(
        None,
        format!("op-pi degraded: source '{source_name}' stopped: {error_message}"),
    )
...
async fn post_native_hook(
    ...
    eprintln!(
        "op-pi native hook received: provider={} event={} repo={} session={}",
        raw_native_field(
            &payload,
    ...
    let event = match incoming_event_from_native_hook_json(&payload) {
        ...
        Err(error) => {
            ...
            eprintln!(
                "op-pi native hook dropped: provider={} event={} reason=normalization_failed error={}",
                raw_native_field(
                    &payload,
                    &["/provider", "/source/provider", "/context/provider"],
                ),
    ...
    if raw_non_git || native_hook_should_drop(&event) {
        ...
        eprintln!(
            "op-pi native hook dropped: {} reason={}",
    ...
    if let Some(defer) = stale_native_replay_defer(&event, &payload) {
        ...
        eprintln!(
            "op-pi native hook deferred: {} reason={} age_secs={}",
            native_event_telemetry_fields(&event),
            defer.reason,
            defer.age.as_secs()
        );
...
fn stale_replay_defer_response(kind: &str, defer: &NativeReplayDefer) -> axum::response::Response {
    eprintln!(
        "op-pi deferred stale replay: type={} reason={} timestamp={} age_secs={}",
        kind,
        defer.reason,
        defer.timestamp,
        defer.age.as_secs()
...
mod tests {
    ...
    fn gajae_test_event() -> IncomingEvent {
        IncomingEvent {
            ...
            format: None,
            template: None,
            payload: json!({"repo": "op-pi", "number": 250}),
        }
    }
    ...
    async fn github_release_retag_routes_to_gajae_hold_without_release_event() {
        ...
        let payload = json!({
            "action": "edited",
            "repository": {"full_name": "IYENTeam/op-pi"},
            "release": {
                "tag_name": "v0.6.9",
                "name": "op-pi 0.6.9",
                "prerelease": false,
                "html_url": "https://github.com/IYENTeam/op-pi/releases/tag/v0.6.9"
            },
        ...
        assert_eq!(queued.payload["repo"], json!("IYENTeam/op-pi"));
        ...
        assert_eq!(
            ...
            json!("IYENTeam/op-pi:owner-maintainer:edited:v0.6.9")
        );
    ...
    async fn github_main_merge_routes_to_gajae_hold_without_merge_event() {
        ...
        let payload = json!({
            ...
            "pull_request": {
                ...
                "html_url": "https://github.com/IYENTeam/op-pi/pull/252",
                "merged": true,
        ...
        assert_eq!(
            queued.payload["dedupe_key"],
            json!(
                "IYENTeam/op-pi:owner-maintainer:merge-to-main:0123456789abcdef0123456789abcdef01234567"
            )
        );
        assert_eq!(queued.payload["autonomous_execution_allowed"], json!(false));
        assert_eq!(queued.payload["held_action_executed"], json!(false));
    ...
    async fn update_status_returns_pending_when_set() {
        ...
        *pending.write().await = Some(update::PendingUpdate {
            ...
            release_url: "https://github.com/IYENTeam/op-pi/releases/tag/v0.6.0".into(),
    ...
    async fn aws_sns_rejects_non_allowlisted_topic() {
        let mut config = AppConfig::default();
        config.aws.topic_allowlist = vec!["arn:aws:sns:us-east-1:123456789012:op-pi-alarms".into()];
