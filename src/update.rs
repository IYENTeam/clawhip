...
pub struct UpdateConfig {
    ...
    /// Absolute path to the op-pi git checkout used for self-update.
...
pub async fn run_checker(
    ...
    let http = reqwest::Client::builder()
        .user_agent(format!("op-pi/{VERSION}"))
        .build()
        .expect("http client");

    println!("op-pi update checker starting (interval: {check_secs}s)");
        ...
        match check_latest_release(&http).await {
            Ok(Some(release)) if version_is_newer(&release.tag_name) => {
                ...
                *pending.write().await = Some(update);
                ...
                let message = format!(
                    "op-pi update available: v{VERSION} \u{2192} v{latest}\n\
                     Approve via: POST /api/update/approve  or  op-pi update approve\n\
                     {}",
                    release.html_url,
                );
                ...
                if let Err(error) = tx.send(event).await {
                    eprintln!("op-pi update checker: failed to send notification: {error}");
                }
            }
            Ok(_) => {}
            Err(error) => {
                eprintln!("op-pi update checker: release check failed: {error}");
            }
        }
    }
}
...
pub async fn approve_update(
    ...
    match &result {
        Ok(()) => {
            let message =
                format!("op-pi updated to v{result_version} successfully. Restart may follow.");
            let event =
                IncomingEvent::custom(channel, message).with_format(Some(MessageFormat::Alert));
            let _ = tx.send(event).await;
        }
        Err(error) => {
            let message = format!("op-pi update to v{result_version} failed: {error}");
            let event =
                IncomingEvent::custom(channel, message).with_format(Some(MessageFormat::Alert));
            let _ = tx.send(event).await;
        }
