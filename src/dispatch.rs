...
impl Dispatcher {
    ...
    fn observe_native_route_outcome(
        ...
        eprintln!(
            "op_pi native hook routed: {} route={} deliveries={} error={}",
    ...
    async fn send_delivery(&self, event: &IncomingEvent, delivery: &ResolvedDelivery) -> bool {
        let Some(sink) = self.sinks.get(delivery.sink.as_str()) else {
            ...
            eprintln!(
                "op_pi dispatcher missing sink '{}' for target {}",
                delivery.sink,
            ...
            .await
        {
            ...
            Err(error) => {
                ...
                eprintln!(
                    "op_pi dispatcher failed to render {} for {}/ {}: {error}",
                    event.canonical_kind(),
                    delivery.sink,
                    safe_target_for_log(&delivery.target)
                );
    ...
    async fn send_routine_batch(&self, batch: FlushedRoutineDeliveryBatch) {
        ...
        let Some(sink) = self.sinks.get(first.delivery.sink.as_str()) else {
            ...
            eprintln!(
                "op_pi dispatcher missing sink '{}' for batched target {}",
                first.delivery.sink,
        ...
        for item in &batch.items {
                ...
                .await
            {
                ...
                Err(error) => {
                    ...
                    eprintln!(
                        "op_pi dispatcher failed to render batched {} for {}/ {}: {error}",
...
mod tests {
    ...
    async fn dispatcher_batches_ci_events_into_single_delivery() {
        ...
        for workflow in ["Build", "Test"] {
            let mut event = IncomingEvent::github_ci(
                ...
                Some("success".into()),
                "abcdef1234567".into(),
                format!("https://github.com/IYENTeam/op_pi/actions/runs/123/jobs/{workflow}"),
    ...
    async fn dispatcher_keeps_ci_events_off_routine_batcher() {
        ...
        for workflow in ["Build", "Test"] {
            let mut event = IncomingEvent::github_ci(
                ...
                Some("success".into()),
                "abcdef1234567".into(),
                format!("https://github.com/IYENTeam/op_pi/actions/runs/456/jobs/{workflow}"),
    ...
    async fn dispatcher_keeps_distinct_event_kinds_in_separate_routine_batches() {
        ...
        assert!(
            first.contains("git:op_pi@main 1234567 ship it")
                || second.contains("git:op_pi@main 1234567 ship it")
        );
        assert!(
            (first.contains("tmux:issue-132 matched 'error' => boom")
                && !first.contains("git:op_pi@main 1234567 ship it"))
    ...
    fn batch_key_prefers_workflow_run_id() {
        ...
        assert_eq!(ci_batch_key(&payload), "op_pi:86:abc:123456789");
    }
