...
mod tests {
    ...
    fn template_context_backfills_repo_and_session_aliases() {
        ...
        assert_eq!(git_context.get("repo").map(String::as_str), Some("op-pi"));
        assert_eq!(
    ...
    fn renders_github_ci_failed_in_compact_and_alert_formats() {
        let event = IncomingEvent::github_ci(
            ...
            "https://github.com/IYENTeam/op-pi/actions/runs/1".into(),
        ...
        assert_eq!(
            event.render_default(&MessageFormat::Compact).unwrap(),
            "CI failed · op-pi#58 · CI / test · failure · abcdef1 · https://github.com/IYENTeam/op-pi/actions/runs/1"
        );
        assert_eq!(
            event.render_default(&MessageFormat::Alert).unwrap(),
            "🚨 CI failed · op-pi#58 · CI / test · failure · abcdef1 · https://github.com/IYENTeam/op-pi/actions/runs/1"
    ...
    fn renders_github_ci_started_with_status_details() {
        ...
        assert_eq!(
            event.render_default(&MessageFormat::Compact).unwrap(),
            "CI started · op-pi#58 · CI / test · in_progress · abcdef1 · https://github.com/IYENTeam/op-pi/actions/runs/1"
        );
        assert_eq!(
            event.render_default(&MessageFormat::Alert).unwrap(),
            "🚨 CI started · op-pi#58 · CI / test · in_progress · abcdef1 · https://github.com/IYENTeam/op-pi/actions/runs/1"
        ...
        let event = normalize_event(IncomingEvent {
            ...
            payload: json!({
                ...
                "context": {
                    ...
                    "repo_name": "op-pi",
                    "repo_path": "/repo/op-pi",
                    "worktree_path": "/repo/op-pi-worktrees/issue-65",
    ...
    fn normalize_event_maps_omc_signal_route_key_into_session_event() {
        let event = normalize_event(IncomingEvent {
            ...
            payload: json!({
                ...
                "signal": {
                    ...
                    "phase": "finished",
                    "summary": "https://github.com/IYENTeam/op-pi/pull/67"
                },
                "context": {
                    "sessionId": "issue-65",
                    "projectPath": "/repo/op-pi-worktrees/issue-65",
                    "projectName": "op-pi"
                }
            }),
        });
        ...
        assert_eq!(event.payload["project"], json!("op-pi"));
        assert_eq!(event.payload["repo_name"], json!("op-pi"));
        assert_eq!(
            event.payload["repo_path"],
            json!("/repo/op-pi-worktrees/issue-65")
        );
        assert_eq!(
            event.payload["worktree_path"],
            json!("/repo/op-pi-worktrees/issue-65")
        ...
        assert_eq!(
            ...
            json!("https://github.com/IYENTeam/op-pi/pull/67")
        );
        assert_eq!(event.payload["status"], json!("finished"));
    }
    ...
    fn normalize_event_maps_omc_native_contract_into_session_event() {
        let event = normalize_event(IncomingEvent {
            ...
            payload: json!({
                ...
                "signal": {
                    ...
                    "summary": "https://github.com/IYENTeam/op-pi/pull/71"
                },
                "context": {
                    "sessionId": "issue-65",
                    "projectPath": "/repo/op-pi",
                    "projectName": "op-pi"
                }
            }),
        });
        ...
        assert_eq!(event.payload["project"], json!("op-pi"));
        assert_eq!(event.payload["repo_name"], json!("op-pi"));
        assert_eq!(event.payload["repo_path"], json!("/repo/op-pi"));
        assert_eq!(event.payload["worktree_path"], json!("/repo/op-pi"));
        assert_eq!(event.payload["tool_name"], json!("Bash"));
        assert_eq!(event.payload["command"], json!("gh pr create"));
        assert_eq!(
            event.payload["summary"],
            json!("https://github.com/IYENTeam/op-pi/pull/71")
        );
    ...
    fn renders_omc_pr_created_event_using_contract_label() {
        ...
        assert_eq!(
            event.render_default(&MessageFormat::Compact).unwrap(),
            "omc issue-65 pr-created (repo=op-pi, issue=#65, pr=#67, summary=https://github.com/IYENTeam/op-pi/pull/67)"
        );
    }
    ...
    fn renders_session_contract_events_in_low_noise_formats() {
        let event = normalize_event(IncomingEvent {
            ...
            payload: json!({
                "context": {
                    ...
                    "issue_number": 65,
                    "pr_number": 71,
                    "pr_url": "https://github.com/IYENTeam/op-pi/pull/71"
                }
            }),
        });
        ...
        assert_eq!(
            event.render_default(&MessageFormat::Compact).unwrap(),
            "omx issue-65 pr-created (repo=op-pi, issue=#65, pr=#71, branch=feat/issue-65-native-event-contract-polish)"
        );
        assert_eq!(
            event.render_default(&MessageFormat::Inline).unwrap(),
            "[omx issue-65] pr-created · op-pi · issue #65 · PR #71 · feat/issue-65-native-event-contract-polish"
        );
    }
