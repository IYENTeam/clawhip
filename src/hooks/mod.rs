...
pub fn install(args: HooksInstallArgs) -> Result<()> {
    ...
    println!("Ingress: op-pi native hook --provider <codex|claude-code>");
...
fn ensure_supported_install_scope(args: &HooksInstallArgs) -> Result<()> {
    ...
    Err(anyhow!(
        "Claude Code provider-native hook forwarding is global-only; Codex may use either ~/.codex/hooks.json or <repo>/.codex/hooks.json with the op-pi bridge in ~/.op-pi"
    )
    .into())
}
