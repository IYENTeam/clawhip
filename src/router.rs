        ...
        S: Sink + ?Sized,
    {
        ...
        for delivery in self.resolve(event).await? {
            ...
            if let Err(error) = sink.send(&delivery.target, &message).await {
                eprintln!(
                    "op-pi router delivery failed to {:?}: {error}",
                    delivery.target
                );
            }
        }
    ...
    fn target_for(
        ...
        match sink {
            "discord" => {
                ...
                // For custom events (e.g. `op-pi send --channel X`), the
...
mod tests {
    ...
    async fn resolve_localfile_route_targets_local_path() {
        let config = AppConfig {
            routes: vec![RouteRule {
                event: "tmux.keyword".into(),
                sink: "localfile".into(),
                local_path: Some("/tmp/op-pi/events.jsonl".into()),
                ..RouteRule::default()
            }],
            ..AppConfig::default()
        };
        ...
        assert_eq!(
            delivery.target,
            SinkTarget::LocalFile("/tmp/op-pi/events.jsonl".into())
        );
    ...
    async fn preview_matches_git_routes_on_worktree_path() {
        ...
        assert_eq!(
            content,
            "git:op-pi[wt:issue-115]@feat/issue-115 1234567 ship it"
        );
    }
    ...
    async fn route_level_mention_is_prepended_for_github_and_tmux() {
        ...
        let github_event =
            IncomingEvent::github_issue_opened("op-pi".into(), 5, "boom".into(), None);
    ...
    async fn agent_family_route_matches_all_agent_events() {
        let config = AppConfig {
            ...
            routes: vec![RouteRule {
                event: "agent.*".into(),
                sink: "discord".into(),
                filter: [("project".to_string(), "op-pi".to_string())]
                    .into_iter()
                    .collect(),
        ...
        let config = AppConfig {
            ...
            routes: vec![RouteRule {
                ...
                filter: [
                    ...
                    ("project".to_string(), "op-pi".to_string()),
                ]
                .into_iter()
                .collect(),
    ...
    async fn native_omc_session_events_match_session_routes() {
        let config = AppConfig {
            ...
            routes: vec![RouteRule {
                ...
                filter: [
                    ...
                    ("repo_name".to_string(), "op-pi".to_string()),
                ]
                .into_iter()
                .collect(),
                channel: Some("session-route".into()),
        ...
        assert!(content.contains("repo=op-pi"));
    ...
    async fn filter_can_route_same_event_type_by_repo() {
        ...
        let router = Router::new(Arc::new(config));
        let event = IncomingEvent::github_issue_opened("op-pi".into(), 7, "bug".into(), None);
    ...
    async fn git_and_github_routes_can_filter_on_repo_name_alias() {
        let config = AppConfig {
            ...
            routes: vec![RouteRule {
                event: "git.commit".into(),
                sink: "discord".into(),
                filter: [("repo_name".to_string(), "op-pi".to_string())]
                    .into_iter()
                    .collect(),
        ...
        let config = AppConfig {
            ...
            routes: vec![
                RouteRule {
                    event: "tmux.*".into(),
                    sink: "discord".into(),
                    filter: [("session_name".to_string(), "op-pi-*".to_string())]
                        .into_iter()
                        .collect(),
    ...
    async fn session_routes_prefer_repo_metadata_over_session_prefix_heuristics() {
        ...
        let event = normalize_event(IncomingEvent {
            ...
            template: None,
            payload: json!({
                "session_name": "op-pi-issue-152",
                "repo_name": "op-pi",
                "worktree_path": "/repo/op-pi.worktrees/issue-152",
            }),
        });
    ...
    async fn custom_event_channel_takes_precedence_over_route_channel() {
        ...
        // op-pi send --channel user-target --message "hello"
    ...
    async fn custom_event_without_channel_falls_back_to_route_then_default() {
        ...
        // op-pi send --message "hello" (no --channel)
    ...
    fn resolve_tmux_session_channel_with_metadata_prefers_repo_route_over_prefix_heuristic() {
        let config = AppConfig {
            ...
            routes: vec![
                ...
                RouteRule {
                    event: "tmux.*".into(),
                    filter: BTreeMap::from([("repo_name".into(), "op-pi".into())]),
                    sink: "discord".into(),
                    channel: Some("metadata-route".into()),
                    ..RouteRule::default()
                },
    ...
    async fn slack_dispatch_posts_block_kit_payload() {
        ...
        router
            .dispatch(
                &IncomingEvent::custom(None, "hello from op-pi".into()),
                &slack,
            )
            .await
            .unwrap();
        ...
        let request = timeout(Duration::from_secs(2), server)
            ...
            .unwrap()
            .unwrap();
        assert!(request.contains("\"text\":\"hello from op-pi\""));
