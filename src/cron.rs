...
fn suppression_notice(job_id: &str, suppression: &ZeroBacklogSuppression, now: i64) -> String {
    ...
    format!(
        "op_pi cron '{job_id}' zero-backlog follow-up suppressed (key={}, expires_in={remaining}s); nudge intentionally withheld, not dropped",
        suppression.key
    )
}
...
fn load_scheduler_state(path: &Path) -> Result<CronSchedulerState> {
    ...
    match serde_json::from_str(trimmed) {
        ...
        Err(error) => {
            eprintln!(
                "op_pi cron state '{}' is invalid; ignoring persisted state: {error}",
                path.display()
            );
