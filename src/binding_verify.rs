...
mod tests {
    ...
    fn collects_route_binding_with_filter() {
        ...
        filter.insert("repo".into(), "op-pi".into());
        let config = config_with_routes(vec![RouteRule {
            event: "*".into(),
            filter,
            channel: Some("222".into()),
            thread: None,
            channel_name: Some("op-pi-dev".into()),
            ..RouteRule::default()
        }]);
        let bindings = collect_bindings(&config);
        assert_eq!(bindings.len(), 1);
        assert_eq!(bindings[0].channel_id, "222");
        assert_eq!(bindings[0].expected_name.as_deref(), Some("op-pi-dev"));
        assert!(bindings[0].label.contains("repo=op-pi"));
    }
    ...
    fn verdict_match_when_hint_matches() {
        let lookup = ChannelLookup::Found {
            id: "1".into(),
            name: Some("op-pi-dev".into()),
        };
        let verdict = resolve_verdict(lookup, &Some("op-pi-dev".into()));
    ...
    fn apply_repo_binding_creates_route() {
        ...
        config
            .apply_repo_binding("op-pi", "123456", Some("op-pi-dev"))
            .unwrap();
        assert_eq!(config.routes.len(), 1);
        assert_eq!(config.routes[0].channel.as_deref(), Some("123456"));
        assert_eq!(config.routes[0].channel_name.as_deref(), Some("op-pi-dev"));
        assert_eq!(config.routes[0].filter.get("repo").unwrap(), "op-pi");
    }
    ...
    fn apply_repo_binding_updates_existing() {
        let mut config = AppConfig::default();
        config
            .apply_repo_binding("op-pi", "111", Some("old-name"))
            .unwrap();
        config
            .apply_repo_binding("op-pi", "222", Some("new-name"))
            .unwrap();
        assert_eq!(config.routes.len(), 1);
